...
//
//	file-uri		= "filereloader:" local-path
...
//
// Examples:
// `filereloader:path/to/file` - relative path (unix, windows)
// `filereloader:/path/to/file` - absolute path (unix, windows)
// `filereloader:c:/path/to/file` - absolute path including drive-letter (windows)
// `filerelaoder:c:\path\to\file` - absolute path including drive-letter (windows)
//
...
// Because Kubernetes controllers typically mount collector configs from a ConfigMap, the mounted file is a symlink. So we watch for
